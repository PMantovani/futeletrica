  ...
  <div className="mx-auto">
    {results.map((result, idx) => (
      <div key={idx} className="relative m-4 flex items-center">
        <TeamContainer color={result.color1} goals={result.goals1} opponentGoals={result.goals2} first={true} />
        <div className=" absolute left-[48.5%] top-[32%] text-neutral-700">x</div>
        <TeamContainer color={result.color2} goals={result.goals2} opponentGoals={result.goals1} first={false} />
      </div>
    ))}
  </div>
);
...
const Goals: React.FC<{ goals: number }> = ({ goals }) => <div className="text-neutral-400">{goals}</div>;
